    ...
    try {
        ...
        for (let i = 0; i < lyrics.length; i += 1000) {
            ...
            const embed = new Discord.MessageEmbed()
                .setTitle("Lista de reprodução personalizada")
...
function lyricsEmbed(lyrics, song) {
  ...
  function replacedefaultmessages(text, o = {}){
      if(!text || text == undefined || text == null) throw "Nenhum texto para a mensagem de padrão substituído adicionado como primeiro parâmetro";
...
function onCoolDown(message, command) {
    if(!message || !message.client) throw "Nenhuma mensagem com um DiscordClient válido concedido como primeiro parâmetro";
    if(!command || !command.name) throw "Nenhum comando com um nome válido concedido como segundo parâmetro";
  ...
  function createBar(total, current, size = 25, line = "▬", slider = "🔷") {
    try {
      if (!total) throw "TEMPO MÁXIMO EXCEDIDO";
   ...
   */
   ...
   async function swap_pages(client, message, description, TITLE) {
     ...
     .setTitle(`${emoji.msg.ERROR}Nenhum conteúdo adicionado à função SWAP PAGES`)
     .setColor(ee.wrongcolor)
     .setFooter(ee.footertext, ee.footericon)]}).catch(e => console.log("ISTO É PARA EVITAR UM CRASH"))
     if (embeds.length === 1) return message.channel.send({embeds: [embeds[0]]}).catch(e => console.log("ISTO É PARA EVITAR UM CRASH"))
     ...
     let swapmsg = await message.channel.send({   
         content: `***Clique nos __Botões__ para trocar as páginas***`,
         embeds: [embeds[0]], 
     ...
     collector.on('collect', async b => {
         if(b.user.id !== message.author.id)
           return b.reply(`<:declined:780403017160982538> **Apenas aquele que digitou $ {prefix} help tem permissão para reagir!**`, true)
           //page forward
           if(b.customId == "1") {
