...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (functions.check_if_dj(message))
            return functions.embedbuilder(client, 6000, message, config.colors.no, "DJ-ROLE", `<a:declined:876968121116807208> Você não têm permissâo para usar esse comando, você precisa ter: ${functions.check_if_dj(message)}`)
        ...
        if (!message.member.voice.channel) return functions.embedbuilder(client, 5000, message, config.colors.no, "`" + message.author.tag + "`" + " Você entrar em um canal de voz")
