...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (functions.check_if_dj(message))
            return functions.embedbuilder(client, 6000, message, config.colors.no, "DJ-ROLE", `<a:declined:876968121116807208> Você não têm prmissão para usar este comando, você precisa ter: ${functions.check_if_dj(message)}`)
        ...
        if (message.member.voice.channel.id != message.guild.me.voice.channel.id) return functions.embedbuilder(client, 5000, message, config.colors.no, "`" + message.author.tag + "`" + " Você deve entrar no mesmo canal de voz que eu: " + ` \`${message.guild.me.voice.channel.name ? message.guild.me.voice.channel.name : ""}\``)
