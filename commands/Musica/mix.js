...
module.exports = {
	...
	description: "Toca uma mixagem definida", //the command description for Slash Command Overview
	cooldown: 2,
	...
	run: async (client, message, args) => {
		try {
			...
			let newMsg = await message.reply({
				content: `${client.allEmojis.loading} Carregando o**'Mix de música ${args[0] ? args[0] : "Predefinido"}'**`,
			});
			try {
				...
				//Edit the reply
				newMsg.edit({
					content: `${queue.songs.length > 0 ? "👍 Carregado" : "🎶 Tocando Agora"}: **'${args[0] ? args[0] : "Predefinido"}'**`,
				});
			} catch (e) {
