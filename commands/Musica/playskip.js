...
module.exports = {
	...
	description: "Toca uma música/lista de reprodução e pula!", //the command description for Slash Command Overview
	cooldown: 2,
	...
	run: async (client, message, args) => {
		try {
			...
			if (!args[0]) {
				return message.reply({
					embeds: [new MessageEmbed()
						...
						.setDescription(`**Use:**\n> \`${client.settings.get(message.guild.id, "prefix")}playskip <Procurar/Link>\``)
					],
				});
			}
			...
			try {
				...
				//Edit the reply
				newmsg.edit({
					content: `${queue.songs.length > 0 ? "⏭ Pulando para" : "🎶 Tocando Agora"}: \`\`\`css\n${Text}\n\`\`\``,
				}).catch(e => {
					console.log(e)
				})
			} catch (e) {
