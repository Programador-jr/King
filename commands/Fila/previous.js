...
module.exports = {
	...
	description: "Toca a música anterior!", //the command description for Slash Command Overview
	cooldown: 10,
		...
		try {
			...
			if (channel.guild.me.voice.channel && channel.guild.me.voice.channel.id != channel.id) {
				return message.reply({
					embeds: [new MessageEmbed()
						...
						.setTitle(`${client.allEmojis.x} Entre no meu Canal!`)
			...
			try {
				...
				if (!newQueue || !newQueue.previousSongs || newQueue.previousSongs.length == 0) return message.reply({
					embeds: [
						new MessageEmbed().setColor(ee.wrongcolor).setTitle(`${client.allEmojis.x} **Não há músicas anteriores!**`)
					],
				...
				message.reply({
					embeds: [new MessageEmbed()
					  .setColor(ee.color)
					  .setTimestamp()
					  .setTitle(`▶️ **Tocando agora a faixa anterior!**`)
