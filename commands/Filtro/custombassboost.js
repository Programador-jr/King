...
module.exports = {
	...
	description: "Define um Bassboost personalizado com ganho!", //the command description for Slash Command Overview
	cooldown: 5,
			...
			try {
				...
				if (!args[0]) {
					return message.reply({
						embeds: [
							...
							.setColor(ee.wrongcolor)
							.setFooter(ee.footertext, ee.footericon)
							.setTitle(`${client.allEmojis.x} **Por favor, adicione um Bassboost-Gain entre 0 e 20!**`)
						],
					})
				}
				...
				if (bass_gain > 20 || bass_gain < 0) {
					return message.reply({
						embeds: [
							...
							.setColor(ee.wrongcolor)
							.setFooter(ee.footertext, ee.footericon)
							.setTitle(`${client.allEmojis.x} **O ganho de Bassboost deve estar entre 0 e 20!**`)
						],
					})
				}
				...
				message.reply({
					embeds: [new MessageEmbed()
					  .setColor(ee.color)
					  .setTimestamp()
					  .setTitle(`♨️ **Bassboost definido para ${bass_gain}!**`)
