...
module.exports = {
	...
	category: "Filtro",
	usage: "speed <valor da velocidade (0 - 20)>",
	aliases: ["customspeed", "changespeed", "cspeed", "mudarvelocidade", "velocidade"],

	description: "Altera a velocidade da música!", //the command description for Slash Command Overview
	cooldown: 5,
			...
			try {
				...
				if (!args[0]) {
					return message.reply({
						embeds: [
							...
							.setColor(ee.wrongcolor)
							.setFooter(ee.footertext, ee.footericon)
							.setTitle(`${client.allEmojis.x} **Adicione um valor de velocidade entre 0+ e 2!**`)
						],
					})
				}
				...
				message.reply({
					embeds: [new MessageEmbed()
					  .setColor(ee.color)
					  .setTimestamp()
					  .setTitle(`♨**Velociade definida para ${speed_amount}!**`)
