...
module.exports = {
    ...
    if (!mention) return message.channel.send(new Discord.MessageEmbed()
		.setTitle('Mencione um usuário para iniciar o jogo')
		.setColor('#ff0000')
		).then(msg => {
      msg.delete({ timeout: 3000 })
      })

    if(message.mentions.users.first().bot) return message.channel.send(new Discord.MessageEmbed()
        .setTitle('Você não pode lutar contra um bot')
        .setColor('#ff0000')
				).then(msg => {
      msg.delete({ timeout: 3000 })
      })

    if(mention.id === message.author.id) return message.channel.send (new Discord.MessageEmbed()
        .setTitle('Você não pode lutar contra si mesmo!')
        .setColor('#ff0000')
    ).then(msg => {
      msg.delete({ timeout: 3000 })
      })

  if (mention.presence.status === "offline") return message.channel.send (new Discord.MessageEmbed()
	.setTitle('Seu oponente deve estar online para duelar com você!')
	.setColor('#ff0000')
	).then(msg => {
      msg.delete({ timeout: 3000 })
      })
