	...
	run: async (client, message, args) => {
		...
		var embedInicial = new Discord.MessageEmbed()
			.setDescription(
				`**📬ㅤOlá ${
					message.author
				}, precisa de ajuda? Aqui estão todos os meus comandos:**`
			)

			.addField(
				'**BASSBOOST INFORMAÇÃO DO FILTRO**',
				`
			...
			.addField(
				...
				`
            >>> existem várias listas de reprodução personalizadas que você pode reproduzir, cada uma com 75 músicas!
...
reproduzi-los por \`${prefix}playlist <Número da playlist>\`
			...
			.addField(
				'**__BOT DE:__**',
				`
            >>> \`KingKiller#1889\` [\`Website\`](https://kingbot.cf) [\`Convite\`](https://discord.com/oauth2/authorize?client_id=794291443454836766&scope=bot&permissions=2102658175)
            `
			)
			.addField(
				'**__Música - fontes suportadas__**',
				`
            >>> \`Youtube\`, \`Spotify\`, \`Soundcloud\`, [\`Mais\`](https://links.musicium.eu), ...
            `)
		...
		let embed1 = new Discord.MessageEmbed()
			.setDescription(
				`<:RolePlay:810231968297517106>ㅤ**ACÃO**:\n
...
• **${config.prefix}timido**`
		...
		let embed2 = new Discord.MessageEmbed()
			.setDescription(
				`<a:fun:810254327372316713>ㅤ**DIVERSÃO**:\n
• **${
					config.prefix
				}8ball** - Faça uma pergunta para o bot e ele irá responder você da Melhor forma possível.
...
• **${
					config.prefix
				}coinflip** - Gire uma moeda e veja se irá cair cara ou coroa!
...
• **${
					config.prefix
				}ship** - ship um casal ou veja qual a probabilidade de você dar certo com seu crush
...
`
		...
		let embed3 = new Discord.MessageEmbed()
			.setDescription(
				`<:game:810254416635494400>ㅤ**GAMES**:\n
• **${config.prefix}aki** - Jogue uma partida com o gênio    
• **${
					config.prefix
				}fight** - lute com alguém do servidor e veja quem sai vitorioso		
...
• **${
		...
		let embed4 = new Discord.MessageEmbed()
			.setDescription(
				`<:photos:810249979905572864>ㅤ**IMAGEM**:\n
• **${config.prefix}bob** - Coloque uma frase dentro do meme Bob Esponja
• **${
					config.prefix
				}changemymind** - Coloque sua frase dentro de uma imagem Change my mind
...
• **${
					config.prefix
				}monkey** - Gera uma imagem do macaquinho "suspeito" com sua frase personalizada
• **${
					config.prefix
				}pablo** - Gera uma imagem do Pablo Escobar solitario com sua frase personalizada
• **${
					config.prefix
				}stonks** - Gera uma imagem stonks com sua frase personalizada
...
• **${config.prefix}wasted** - Retornna uma imagem wasted!`
		...
		let embed5 = new Discord.MessageEmbed()
			.setDescription(
				`<a:moderation:810250044888317952>ㅤ**MODERAÇÃO**:\n
...
• **${
					config.prefix
				}mute** - Silencie um membro do servidor ideal para aqueles que gostam de quebrar as regras
• **${config.prefix}unmute** - Desmutar um membro do servidor
• **${
					config.prefix
				}tempmute** - Defina o tempo que um membro vai permanecer mutado
...
• **${config.prefix}resetwarns** - Redefinir avisos da pessoa mencionada`
		...
		let embed6 = new Discord.MessageEmbed()
			.setDescription(
				`<:musica:810233712334995496>ㅤ**MUSICA**:\n
• **${config.prefix}aleatorio** - Colocar a fila no aleatorio
• **${
					config.prefix
				}fila** - Mostra a fila de músicas e o que está tocando agora.
...
• **${config.prefix}volume** - "Alterar o volume atual no servidor`
		...
		let embed7 = new Discord.MessageEmbed()
			.setDescription(
				`<:nsfw2:810232533462810666>ㅤ**NSFW**:\n
• **${config.prefix}4k**
• **${config.prefix}anal**
• **${config.prefix}ass**
• **${config.prefix}gif**
...
• **${config.prefix}thigh**`
		...
		let embed8 = new Discord.MessageEmbed()
			.setDescription(
				`<:utility:810250009894584351>ㅤ**UTILIDADE**:\n
...
• **${
					config.prefix
				}convite** - Para me adicionar / convidar o bot para o seu servidor
• **${
					config.prefix
				}covid** - Receba atualizações de todo o mundo dos casos de covid 19
...
• **${
					config.prefix
				}serveravatar** - Retorna o logo do servidor sendo possivel fazer o download do mesmo
• **${config.prefix}serverinfo** - Receba as informações do seu servidor
• **${
					config.prefix
				}sugerir** - Enviar uma sugestão para o seu canal de sugestões
• **${config.prefix}tradutor** - Traduza uma frase ou um texto para o seu idioma
• **${
					config.prefix
				}userinfo** - Obtenha estatísticas avançadas de determinada pessoa ou de você mesmo`
			)
		...
		message.channel.send(embedInicial).then(async msg => {
			...
			const inicio = msg.createReactionCollector(
				(r, u) =>
					r.emoji.id === '810254382549565530' && u.id === message.author.id,
				{ time: 60000 }
			);
			const aca = msg.createReactionCollector(
				(r, u) =>
					r.emoji.id === '810231968297517106' && u.id === message.author.id,
				{ time: 120000 }
			);
			const diver = msg.createReactionCollector(
				(r, u) =>
					r.emoji.id === '810254327372316713' && u.id === message.author.id,
				{ time: 120000 }
			);
			const game = msg.createReactionCollector(
				(r, u) =>
					r.emoji.id === '810254416635494400' && u.id === message.author.id,
				{ time: 120000 }
			);
			const image = msg.createReactionCollector(
				(r, u) =>
					r.emoji.id === '810249979905572864' && u.id === message.author.id,
				{ time: 120000 }
			);
			const mod = msg.createReactionCollector(
				(r, u) =>
					r.emoji.id === '810250044888317952' && u.id === message.author.id,
				{ time: 120000 }
			);
			const music = msg.createReactionCollector(
				(r, u) =>
					r.emoji.id === '810233712334995496' && u.id === message.author.id,
				{ time: 120000 }
			);
			const nsfw = msg.createReactionCollector(
				(r, u) =>
					r.emoji.id === '810232533462810666' && u.id === message.author.id,
				{ time: 120000 }
			);
			const util = msg.createReactionCollector(
				(r, u) =>
					r.emoji.id === '810250009894584351' && u.id === message.author.id,
				{ time: 120000 }
			);
