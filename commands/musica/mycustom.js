...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (args[0] === "add" || args[0] === "set" || args[0] === "use") {
      ...
      if (playlist.includes(args[1])) return message.reply("Ah não! A música já existe na lista de reprodução do servidor, SEM MÚSCAS DUPLICADAS!")
      ...
      Há agora: \`${playlist.length+1}\` músicas na lista de reprodução do servidor`)
    }
    if (args[0] === "reset" || args[0] === "apagar") {
      let themsg = await message.reply("ocê realmente deseja redefinir sua lista de reprodução personalizada? || (* Responder com: * ** __ `sim`__**)||")
      ...
      themsg.channel.awaitMessages(filter, {
        ...
        .then(async collected => {
          if (collected === "sim") {
            ...
            await message.reply("REINICIOU COM SUCESSO SUA PLAYLIST PERSONALIZADA")
          }
        }).catch(error => {
    ...
    if (args[0] === "play" || args[0] === "p" || args[0] === "hear" || args[0] === "listen") {
      ...
      return functions.embedbuilder(client, 5000, message, config.colors.yes, "JOGANDO LISTA DE REPRODUÇÃO PERSONALIZADA")
    }
    if (args[0] === "remover" || args[0] === "deletar" || args[0] === "del" || args[0] === "rem") {
      ...
      return functions.embedbuilder(client, 5000, message, config.colors.yes, "REMOVEU A MÚSICA COM SUCESSO DA SUA PLAYLIST")
    } else {
      let string = playlist.join("\n");
      customplay(message, string, playlist)
      functions.embedbuilder(client, "null", message, config.colors.yes, `Existe ${playlist.length} Músicas em sua lista de reprodução!`, )
      return functions.embedbuilder(client, "null", message, config.colors.yes, `Sintaxe do comando: `," + mycustom <adicionar / remover / reproduzir> [Link]")
    }
  }
};
